export const eqData = [
  {
    img: 'https://res.cloudinary.com/samtufail726/image/upload/v1675558002/kodiak/Shop/IMG_5486_jacxcc.jpg',
    title: 'Sharpening Stone',
    txt: `'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries,'`,
    price: '12.00',
  },
  {
    img: 'https://res.cloudinary.com/samtufail726/image/upload/v1675557997/kodiak/Shop/IMG_5460_btyftn.jpg',
    title: 'CLEANING KIT',
    txt: `'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries,'`,
    price: '12.00',
  },
  {
    img: '/images/shop-all/tongass.png',
    title: 'THE TONGASS',
    txt: `'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries,'`,
    price: '12.00',
  },
  {
    img: 'https://res.cloudinary.com/samtufail726/image/upload/f_auto,q_auto/v1675502320/kodiak/Shop/Equipment/pixelcut_1_gcq4o0.webp',
    title: 'THE LECONTE',
    txt: `'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries,'`,
    price: '12.00',
  },
];
