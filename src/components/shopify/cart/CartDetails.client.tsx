...
export function CartDetails({ layout, onClose }: { layout: 'drawer' | 'page'; onClose?: () => void }) {
  ...
  return (
    <form className={container[layout]} onSubmit={(evt) => evt.preventDefault()}>
      <section ref={scrollRef} aria-labelledby="cart-contents" className={`${content[layout]} ${y > 0 ? 'border-t' : ''}`}>
