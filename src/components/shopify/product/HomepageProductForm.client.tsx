...
export function HomepageProductForm() {
  //   const { pathname, search } = useUrl();
  //   const [messageApi, contextHolder] = message.useMessage();
  //   const [params, setParams] = useState(new URLSearchParams(search));

  const { options, setSelectedOption, selectedVariant } =
  ...
  useEffect(() => {
    (options as OptionWithValues[]).map(({ name, values }) => {
      ...
      // );
      setSelectedOption(name, values[0]);
      //   } else {
      // params.set(
    ...
    <>
      ...
      <form className="grid">
        ...
        <div className="grid gap-10">
          ...
          <div className="grid gap-4 2xl:max-w-[600px]">
            {/* <AddToCartButton
          ...
          >
            ...
            {!isOutOfStock && (
              <BuyNowButton
                variantId={selectedVariant.id}
                quantity={1}
                className="text-white bg-[#879281] py-[1rem] rounded-[80px]"
              >
                <Text
                  as="span"
                  className="flex items-center justify-center gap-2 font-[700] uppercase"
